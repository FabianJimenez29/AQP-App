/**
 * Pantalla de Vista Previa del Reporte antes de generar PDF
...
const ReportPreviewScreen: React.FC<ReportPreviewScreenProps> = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const loadLogo = async () => {
      try {
        ...
        console.log('🎨 Cargando logo:', isCartapate ? 'Cartapate' : 'AquaPool', 'para usuario:', user?.email);
  ...
  /**
   * Finaliza y vuelve al dashboard
   */
  const handleFinish = () => {
    Alert.alert(
      'Finalizar',
      '¿Deseas cerrar la vista previa y volver al inicio?',
      [
